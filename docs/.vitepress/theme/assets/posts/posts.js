...
export function getPostList(currentPage, filter = null) {
    const currentPagePostListPromise = getFullPostList().then(fullPostList => {
    ...
    return currentPagePostListPromise;
}
...
export function getPostCount(filter = null) {
    const postCountPromise = getFullPostList().then(fullPostList => {
        if (filter) {
            let count = 0;
...
export function getPostInfo(postId) {
    const postInfoPromise = getFullPostList().then(fullPostList => {
        return fullPostList.find(post => post.ID === postId);
    });
    return postInfoPromise;
}
