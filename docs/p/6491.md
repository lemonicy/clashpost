...
- 匹配到对手后，系统会使用这一刻你所选阵型的“快照”作为防守阵型，此“快照”的功能与 [部落战](/p/588) 和 [部落对战联赛](/p/833) 相同。其他玩家攻击的就是这个“快照”。
  - 这也就是说，在突袭期间修改阵型不会影响当前对手看到的阵型，但是从下一个部落开始，对手看到的阵型就是新的了。
  - 如果在双方匹配到的那一刻建筑正在升级，则建筑会以升级前的状态参战，参战时建筑会保留完整功能，并且计入摧毁率。
  - 如果在双方匹配到的那一刻某个建筑的废墟尚未重建完成，则该建筑会保持废墟状态，不会阻碍敌方部队移动，不需要摧毁即可达到 100% 摧毁率。
- 只有首领和副首领可以修改都城的阵型。
...
- 游戏的都城金币奖励与战斗结束后的剩余部队人口和部队剩余生命值有关，详见这篇文章：[部落都城突袭奖章计算公式解析](/p/5001)。
